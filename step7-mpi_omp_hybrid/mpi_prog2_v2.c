...
int main() {
    ...
    // Pour le processus de rang 0, recevoir les résultats des autres processus
    if (myrank == 0) {
        // Déjà calculé la première moitié, recevoir la seconde moitié
        for (int i = N / 2; i < N; i++) {
            MPI_Recv(res[i], N, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &status);
        }
    }
